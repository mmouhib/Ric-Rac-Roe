...
const Cell = (props) => {
	return <div id={props.id} onClick={() => {props.affectChoice(props.id)}} className='cell'>{props.value}</div>;
};
...
const compareElements = (fstElement, secElement, thirdElement, fourthElement) => {
	return ((fstElement === secElement) && (secElement === thirdElement)
		&& (thirdElement === fourthElement));
}

const winner = (board, logo) => {
	return (compareElements(board[0], board[1], board[2], logo)) ||
...
const Board = () => {
	...
	const affectChoice = (id) => {
		setBoard(board.map((element, index) => {
			if (index === id){
				let symbol = logo
				if (logo === 'X'){
					setLogo('O')
